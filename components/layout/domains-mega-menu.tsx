...
function PromoPanel({
  ...
  return (
    <div
      className={cn(
        "relative flex min-h-[220px] flex-col justify-between overflow-hidden bg-[linear-gradient(155deg,#2563eb_0%,#4f46e5_52%,#7c3aed_100%)] p-6 text-white sm:p-7",
        compact ? "rounded-[22px]" : "h-full",
      )}
    >
      <div
        aria-hidden
      ...
      <div className="relative">
        ...
        <p className="mt-2.5 max-w-[34ch] text-[13px] leading-relaxed text-white/85">
          Register a domain, add hosting, and go live from one HostingBeyond
          account — not three different vendors.
        </p>
      </div>
...
export function DomainsMegaMenu({
  ...
  return (
    <div
      className={cn(
        "relative overflow-hidden border border-white/80 bg-white shadow-[0_28px_70px_-28px_rgba(37,80,130,0.45),0_12px_32px_-18px_rgba(15,23,42,0.12)]",
    ...
    >
      <div
        className={cn(
          ...
          compact
            ? "grid-cols-1"
            : "sm:grid-cols-2 lg:gap-8 lg:pr-[min(36%,400px)]",
        )}
      >
        {GROUPS.map((group) => (
          <div key={group.id}>
      ...
      <div
        className={cn(
          compact
            ? "p-4 pt-0"
            : "lg:absolute lg:inset-y-0 lg:right-0 lg:w-[min(36%,400px)]",
        )}
      >
        <PromoPanel compact={compact} onNavigate={onNavigate} />
      </div>
    </div>
  );
}
